                ...
                match current.poll() {
                    ...
                    Err(e) => {
                        ...
                        if let Some(addr) = self.addrs.next() {
                            ...
                            unimplemented!();
//                            *current = TcpStream::connect_with_local_address(&addr, &local, handle);
//                            continue;
                        }
                    }
                }
                ...
                debug!("connecting to {}", addr);
                // OPTION: Do it in tokio-core
//                self.current = Some(TcpStream::connect_with_local_address(&addr, &local, handle));

//                // OPTION: do it directly in Hyper. Tediously requires a boxfuture in self.current.
...
//                    Ok(tcp) => {
//                        self.current = Some(TcpStream::connect_stream(tcp, &addr, handle));
//                        continue;
//                    },
//                    Err(e) => {
//                        err = Some(e)
...
//                };
                ...
                // OPTION: do it directly in Hyper with seanmonstar's suggestion.
                let do_bind = |local: &Option<IpAddr>| {
                    let sock = match addr {
                        SocketAddr::V4(..) => TcpBuilder::new_v4(),
                        SocketAddr::V6(..) => TcpBuilder::new_v6(),
